...
///////////////////////////////////////////////////////////////////////////////
// Code copied from TestSimulation.cpp
namespace SimTK {
...
void testMeasureImplementation()
{
    TestSystem sys;
    TestSystem sysd;
    TestSubsystem subsys(sys);
    TestSubsystem subsysd(sysd);

    // Add a Result measure to the system subsystem. This depends on
    // Position stage and invalidates Dynamics and later stages.
    Measure_<Vector_<double>>::Result vectorResult(sys.updSystemSubsystem(),
        Stage::Position, Stage::Dynamics);
    Measure_<Vector_<adouble>>::Result vectorResultd(sysd.updSystemSubsystem(),
        Stage::Position, Stage::Dynamics);

    MeasureIndex vectorResultIx = vectorResult.getSubsystemMeasureIndex();
    MeasureIndex vectorResultIxd = vectorResultd.getSubsystemMeasureIndex();
    SimTK_TEST_EQ(vectorResultIx, vectorResultIxd)

    Measure_<Vector_<double>>::Result myVecRes = Measure_<Vector_<double>>::
        Result::getAs(sys.updSystemSubsystem().getMeasure(vectorResultIx));
    Measure_<Vector_<adouble>>::Result myVecResd = Measure_<Vector_<adouble>>::
        Result::getAs(sysd.updSystemSubsystem().getMeasure(vectorResultIxd));

    Measure_<double>::Result result(sys.updSystemSubsystem(),
        Stage::Time, Stage::Position);
    Measure_<adouble>::Result resultd(sysd.updSystemSubsystem(),
        Stage::Time, Stage::Position);

    Measure_<double>::Result autoResult(sys.updSystemSubsystem(),
        Stage::Time, Stage::Position);
    Measure_<adouble>::Result autoResultd(sysd.updSystemSubsystem(),
        Stage::Time, Stage::Position);

    autoResult.setIsPresumedValidAtDependsOnStage(true);
    autoResultd.setIsPresumedValidAtDependsOnStage(true);

    Measure_<double>::Zero zero(subsys);
    Measure_<adouble>::Zero zerod(subsysd);

    Measure_<double>::Constant three(subsys,3);
    Measure_<adouble>::Constant threed(subsys,3);

    Measure_<Vec3>::Constant v3const(subsys,Vec3(1,2,3));
    Measure_<Vec3>::Constant v3constd(subsys,Vec3(1,2,3)); // TODO need to create a Vec3 of adouble

    Measure_<double>::Sinusoid cos2pit(subsys,1,2*Pi,Pi/2);
    Measure_<adouble>::Sinusoid cos2pitd(subsys,1,2*Pi,Pi/2);

    // skip the integrate/differentiate steps

    Measure_<double>::Time tMeasure;
    Measure_<adouble>::Time tMeasured;
...
int main() {
    SimTK_START_TEST("TestADOLCCommon");
        ...
        SimTK_SUBTEST(testMeasureImplementation);
    SimTK_END_TEST();
}
