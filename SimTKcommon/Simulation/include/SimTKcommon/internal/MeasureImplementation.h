...
public:
    ...
    template <> class Measure_Num<adouble> {
    public:
        typedef adouble Element;
        static int size(const adouble&) {return 1;}
        static const adouble& get(const adouble& v, int i)
        {  assert(i==0); return v; }
        static adouble& upd(adouble& v, int i) {assert(i==0); return v;}
        static void makeNaNLike(const adouble&, adouble& nanValue)
        {  nanValue = CNT<adouble>::getNaN(); }
        static void makeZeroLike(const adouble&, adouble& zeroValue)
        {  zeroValue=0.; }
    };
#endif
...
private:
                      ...
                      const typename Measure_Num<T>::Element& oldExtreme) const
    {
        ...
        case Extreme::MaxAbs:
            return NTraits<Real>::abs(newVal) > NTraits<Real>::abs(oldExtreme);
        case Extreme::MinAbs:
            return NTraits<Real>::abs(newVal) < NTraits<Real>::abs(oldExtreme);
        };
