...
#include "simbody/internal/Force_TestParallelForce.h"

#include "ForceImpl.h"

namespace SimTK {

class Force::TestParallelForceImpl : public ForceImpl {
    struct InstanceVars {
...
public:
    TestParallelForceImpl(const MobilizedBody& body1, const Transform& frameOnB1, 
                      const MobilizedBody& body2, const Transform& frameOnB2, 
                      const Vec6& stiffness, const Vec6& damping);
    TestParallelForceImpl* clone() const {
        return new TestParallelForceImpl(*this);
    }
    ...
    void realizeTopology(State& s) const {
        TestParallelForceImpl* mThis = const_cast<TestParallelForceImpl*>(this);
...
friend class Force::TestParallelForce;
...
inline std::ostream& operator<<
   (std::ostream& o, const Force::TestParallelForceImpl::InstanceVars& iv)
{   assert(!"implemented"); return o; }
inline std::ostream& operator<<
   (std::ostream& o, const Force::TestParallelForceImpl::PositionCache& pc)
{   assert(!"implemented"); return o; }
inline std::ostream& operator<<
   (std::ostream& o, const Force::TestParallelForceImpl::VelocityCache& vc)
{   assert(!"implemented"); return o; }
inline std::ostream& operator<<
   (std::ostream& o, const Force::TestParallelForceImpl::ForceCache& fc)
{   assert(!"implemented"); return o; }
...
SimTK_INSERT_DERIVED_HANDLE_DEFINITIONS(Force::TestParallelForce, 
                                        Force::TestParallelForceImpl, Force);

Force::TestParallelForce::TestParallelForce
   (GeneralForceSubsystem& forces, 
    const MobilizedBody& body1, const Transform& frameOnB1,
    const MobilizedBody& body2, const Transform& frameOnB2,
    const Vec6& stiffness,  const Vec6&  damping) 
:   Force(new TestParallelForceImpl(body1,frameOnB1,body2,frameOnB2,
                                stiffness,damping))
{
    SimTK_ERRCHK_ALWAYS(stiffness >= 0,
        "Force::TestParallelForce::ctor()",
...
Force::TestParallelForce::TestParallelForce
   (GeneralForceSubsystem& forces, 
    ...
    const Vec6& stiffness,  const Vec6&  damping) 
:   Force(new TestParallelForceImpl(body1,Transform(),body2,Transform(),
                                stiffness,damping))
{
    SimTK_ERRCHK_ALWAYS(stiffness >= 0,
        "Force::TestParallelForce::ctor()",
...
Force::TestParallelForce& Force::TestParallelForce::
...
setDefaultStiffness(const Vec6& stiffness) {
    SimTK_ERRCHK_ALWAYS(stiffness >= 0,
        "Force::TestParallelForce::setDefaultStiffness()",
        "Bushing spring constants must be nonnegative.");
    getImpl().invalidateTopologyCache();
...
setStiffness(State& state, const Vec6& stiffness) const {
    SimTK_ERRCHK_ALWAYS(stiffness >= 0,
        "Force::TestParallelForce::setStiffness()",
        "Bushing spring constants must be nonnegative.");
...
setDamping(State& state, const Vec6& damping) const {
    SimTK_ERRCHK_ALWAYS(damping >= 0,
        "Force::TestParallelForce::setDamping()",
        "Bushing damping coefficients must be nonnegative.");
...
const SpatialVec& Force::TestParallelForce::
...
//---------------------------- TestParallelForceImpl -------------------------------
//------------------------------------------------------------------------------


Force::TestParallelForceImpl::TestParallelForceImpl
   (const MobilizedBody& body1, const Transform& frameOnB1,
    const MobilizedBody& body2, const Transform& frameOnB2,
    const Vec6& stiffness, const Vec6& damping)
...
void Force::TestParallelForceImpl::
...
Real Force::TestParallelForceImpl::
