...
realizeSubsystemInstanceImpl(const State& s) const {
    ...
    for (MobilizedBodyIndex i(0); i < mobilizedBodies.size(); ++i) {
        const Transform& X_BM = iv.outboardMobilizerFrames[i];
        const Transform& X_PF = iv.inboardMobilizerFrames[i];
        ic.outboardMobilizerFramesInverse[i] = ~X_BM;

        SBInstancePerMobodInfo& info = ic.updMobodInstanceInfo(i);
        info.noX_MB = (X_BM.p() == Vec3(0)) && (X_BM.R() == Mat33(1));
        info.noR_PF = (X_PF.R() == Mat33(1));
    }
    ...
    for (MobilizedBodyIndex mbx(0); mbx < getNumBodies(); ++mbx) {
        ...
        iv.prescribedMotionIsDisabled[mbx] =
            mobod.hasMotion() ? mobod.getMotion().isDisabledByDefault()
                              : false;
        iv.inboardMobilizerFrames[mbx] = mobod.getDefaultInboardFrame();
        iv.outboardMobilizerFrames[mbx] = mobod.getDefaultOutboardFrame();
    }
