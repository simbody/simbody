...
template <class T> void assertEqual(T val1, T val2) {
  ASSERT(abs(val1 - val2) < TOL);
}

template <int N> void assertEqual(Vec<N> val1, Vec<N> val2) {
  for (int i = 0; i < N; ++i)
...
static void compareToTranslate(bool prescribe, Motion::Level level) {
  ...
  matter.calcMobilizerReactionForcesUsingFreebodyMethod(state,
                                                        reactionForcesFreebody);
  ...
  for (int i = 0; i < numBodies; i++) {
    ...
    for (MobilizerUIndex j(0); j < 3; ++j) {
      ...
      SimTK_TEST_EQ(body1.getH_FMCol(state, j), body2.getH_FMCol(state, j));
    }
  }
}

static void testFree() { compareToTranslate(false, Motion::Position); }
