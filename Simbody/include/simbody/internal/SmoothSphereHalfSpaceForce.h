 ...
 * contact between a sphere and a half space, where the half space is defined
 * as x > 0 (that is, all coordinates with positive x are in contact) in half
 * space's frame. Commonly, the gravity acceleration vector is (0, -g, 0)
 * (g > 0), and the half space represents ground. To achieve this, the half
 * space should be rotated -90 degrees about the Z-Axis. This can be done by
 * defining the half space frame as:
 * @code
 ...
 * periods with and without sphere-half space contact, we use a tanh function:
 ...
 * where normal determines the direction of the normal to the contact half
 * space (the normal points in the direction of contact).
 *
 * <h1>Potential energy</h1>
 *
 * The potential energy is the integral of the normal restoring force (Hertz
 * force). Due to the smooth approximation, there is no exact expression for
 * the potential energy. We therefore made a comprise by providing an
 * approximation for the potential energy. Specifically, we used the original
 * expression but replaced the original Hertz force by our smooth approximation
 * as follows:
 * <pre> pe = (2/5)*fh_smooth*x </pre>
 * This approximation results in the error depicted in the graph below when
 * differentiating the potential energy with respect to the penetration, the
 * user should thus be aware of the limitations of this model that was
 * primarily design for optimization problems.
 *
