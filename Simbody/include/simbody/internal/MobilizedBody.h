...
acceleration to zero, depending on the \a level parameter. The prescribed 
position or velocity is taken from the \a state and recorded. If the 
mobilizer was already locked, the new lock specification overrides it.
The \a state must already have been realized to at least Stage::Model and 
...
/** Check whether this mobilizer is currently locked in the given \a state. **/
bool isLocked(const State& state) const 
{   return getLockLevel(state)!=Motion::NoLevel; }
/** Returns the lock level if the mobilizer is locked in the given \a state, 
...
motion is unlocked, use Motion::NoLevel as the \a level. This is a topological
...
level, the required q or u value is recorded at the time a \a state is
