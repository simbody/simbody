#ifndef SimTK_SIMBODY_FORCE_TEST_PARALLEL_FORCE_H_
#define SimTK_SIMBODY_FORCE_TEST_PARALLEL_FORCE_H_
 ...
 * subclass Force::TestParallelForce and is logically part of Force.h. The file 
 ...
 * \par Theory:
 *
 * The TestParallelForce calculates the relative orientation X_FM between the 
 ...
 * TestParallelForce force allocates a state variable for this purpose so tracks
 ...
 */
class SimTK_SIMBODY_EXPORT Force::TestParallelForce : public Force {
public:
    /// Create a TestParallelForce between a pair of arbitrary frames, one fixed 
    ...
    ///     translational.
    TestParallelForce(GeneralForceSubsystem& forces, 
    ...
    /// Create a TestParallelForce connecting the body frames of two bodies.
    ...
    /// - The set...() methods return a reference to "this" TestParallelForce (in
    ...
    TestParallelForce& setDefaultFrameOnBody1(const Transform& X_B1F);
    ...
    TestParallelForce& setDefaultFrameOnBody2(const Transform& X_B2M);
    ...
    TestParallelForce& setDefaultStiffness(const Vec6& stiffness);
    ...
    TestParallelForce& setDefaultDamping(const Vec6& damping);
    ...
    ///   TestParallelForce (in the manner of an assignment operator, except
    ...
    const TestParallelForce& setFrameOnBody1(State&             state, 
    ...
    ///     A const reference to "this" Linear Bushing for convenient
    ///     chaining of set...() methods in a single expression.
    const TestParallelForce& setFrameOnBody2(State&             state, 
    ...
    ///     A const reference to "this" Linear Bushing for convenient
    ///     chaining of set...() methods in a single expression.
    const TestParallelForce& setStiffness(State&        state, 
    ...
    ///     A const reference to "this" Linear Bushing for convenient
    ///     chaining of set...() methods in a single expression.
    const TestParallelForce& setDamping(State&      state, 
    ...
    SimTK_INSERT_DERIVED_HANDLE_DECLARATIONS
       (TestParallelForce, TestParallelForceImpl, Force);
    /// @endcond
};

} // namespace SimTK

#endif // SimTK_SIMBODY_FORCE_TEST_PARALLEL_FORCE_H_
