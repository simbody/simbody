...
int
IVM::defaultVecVec3Size(const CDSVector<CDSVec3>& v) {
    return v.size(); // FIX: *3???
}

double
IVM::defaultVecVec3Prod(const CDSVector<CDSVec3>& v1, const CDSVector<CDSVec3>& v2) {
...
IVM::resetCM()
{
    ...
    for (int i=1 ; i<tree()->nodeTree.size() ; i++)
        for (int j=0 ; j<tree()->nodeTree[i].size() ; j++) {
            ...
            a->pos = acCOM_G;
            a->vel = CDSVec3( ConstRSubVec6(acSpatialVel,3,3).vector() )
    ...
    CDSVec3 L(0.0); // angular momentum excluding internal rotations
    ...
    CDSVec3 omega(0.0); // the overall rotational velocity
    if ( nodes.size() >2 ) {
