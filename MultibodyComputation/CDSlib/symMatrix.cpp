...
{
 int exit=0;
 cout << "testing SymMatrix...";

 SymMatrix<double> l(3);              //test all constructors
 SymMatrix<double> l1(2,9.);
 double ar[] = { 4, 
                 2, 5, 
		 6, 7, 8 }; // from row-major array (lower- tri)
 SymMatrix<double> l2(3,ar);
 SymMatrix<double> l3(l2);
 ...
 for (int i=0 ; i<2 ; i++)
   for (int j=0 ; j<2 ; j++)
     if ( fabs(l1(i,j)-9.)>tol ) {
       cerr << "index failed: l1 = " << l1 << " != { {9, 9},{9, 9} }\n";
       exit=1;
     }
 ...
 if (fabs(l2(0,0)+.375)>tol||fabs(l2(0,1)-.625)>tol||fabs(l2(0,2)+.125)>tol ||
     fabs(l2(1,0)-.625)>tol||fabs(l2(1,1)+.375)>tol||fabs(l2(1,2)+.125)>tol ||
     fabs(l2(2,0)+.125)>tol||fabs(l2(2,1)+.125)>tol||fabs(l2(2,2)-.625)>tol ) {
   cerr << "inverse failed: " << l2 << "\n != \n"
	<< "{{-.375 , .625 ,-.125},"
	<< "{ .625 ,-.375 ,-.125}, {-.125 ,-.125 , .625}\n";
