...
public:
    ...
    SimTK_SIMBODY_API explicit RotationMat(const UnitVec3& z);
...
public:
    ...
    Transform(const RotationMat& R, const Vec3& T) : R_BF(R), T_BF(T) { }
    explicit Transform(const RotationMat& R)       : R_BF(R), T_BF(0) { }
    explicit Transform(const Vec3& T)              : R_BF(),  T_BF(T) { }
...
public:
    ...
    // for a Transform (or for that matter, a RotationMat): 15 flops.
...
// These had to wait for InverseTransform to be declared.
