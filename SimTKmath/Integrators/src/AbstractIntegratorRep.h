...
//==============================================================================
//                         ABSTRACT INTEGRATOR REP
//==============================================================================
/* This abstract class implements most of the generic functionality needed for 
an Integrator, leaving only the actual integration method to be implemented 
...
public:
    ...
    Integrator::SuccessfulStepStatus 
        stepTo(double reportTime, double scheduledEventTime) override;

    double getActualInitialStepSizeTaken() const override
    {   return m_actualInitialStepSizeTaken; }

    double getPreviousStepSizeTaken() const override
    {   return m_lastStepSize; }

    SystemYIndex getPreviousStepWorstState() const override
    {   return m_lastWorstState; }

    double getPredictedNextStepSize() const override
    {   return m_currentStepSize; }
...
protected:
    ...
    (including their effect on the error estimate) prior to returning. The 
    default implementation calls the "raw" ODE integrator and then handles the 
    necessary projections; if that's OK for your method then you only have to 
    implement attemptODEStep(). Otherwise, you should override the 
    attemptDAEStep() and deal carefully with the DAE-specific issues yourself. 
    ...
    int statsStepsTaken, statsStepsAttempted, statsErrorTestFailures, 
...
private:
    bool takeOneStep(double tMax, double tReport);

    // Integration method properties.
    int             m_minOrder, m_maxOrder;
    std::string     m_methodName;
    bool            m_hasErrorControl;

    // Runtime.
    bool            m_initialized;
    double          m_currentStepSize, 
                    m_lastStepSize, 
                    m_actualInitialStepSizeTaken;

    SystemYIndex    m_lastWorstState; // which Y drove the step size?
};
