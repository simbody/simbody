...
error estimate and higher order result. See Solving Ordinary Differential
Equations I: Nonstiff Problems, 2nd ref. ed., Hairer, Norsett, & Wanner,
section II.4, pp 164-165.
...
ways: <pre>
    ...
    q1 = q0 + h N(q0)u1         and      qb = q0 + 2h N(q0)u2'
...
Hairer suggests using that error term to improve on y2 to
get a better estimate for y (this is called "local extrapolation") : <pre>
    y2' = y2 + (y2-yb) = 2 y2 - yb
...
accurate. In fact, although this sounds
lovely in theory, in practice we will often be running at close to a stability 
boundary and using the "big step" value to update the result causes stability
problems. So we'll skip the local extrapolation and stick with the first order
result from the two half-steps.
